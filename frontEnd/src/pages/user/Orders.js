              ...
              <React.Fragment key={indx}>
                <div className="flex max-md:flex-col items-center justify-between px-3 md:px-11">
                  <div className="data">
                    ...
                    <p className="font-medium text-lg leading-8 text-black mt-3 whitespace-nowrap">Order Payment : {new Date(ord?.createdAt).toLocaleDateString()}</p>
                  </div>
                ...
                {ord?.products?.map((p) => (
                  <div className="flex max-lg:flex-col items-center gap-8 lg:gap-24 px-3 md:px-11" key={p?._id}>
                    <div className="grid grid-cols-4 w-full">
                      <div className="col-span-4 sm:col-span-1">
                        <img src={`http://localhost:8080/api/v1/product/get-productphoto/${p?._id}`} alt={p.name} className="max-sm:mx-auto object-cover" />
                      </div>
                      <div className="col-span-4 sm:col-span-3 max-sm:mt-4 sm:pl-8 flex flex-col justify-center max-sm:items-center">
                        <h6 className="font-manrope font-semibold text-2xl leading-9 text-black mb-3 whitespace-nowrap">{p.name}</h6>
                        <p className="font-normal text-lg leading-8 text-gray-500 mb-8 whitespace-nowrap">By: {p.brand}</p>
                        <div className="flex items-center max-sm:flex-col gap-x-10 gap-y-3">
                          <span className="font-normal text-lg leading-8 text-gray-500 whitespace-nowrap">Size: {p.size}</span>
                          <span className="font-normal text-lg leading-8 text-gray-500 whitespace-nowrap">Qty: {p.quantity}</span>
                          <p className="font-semibold text-xl leading-8 text-black whitespace-nowrap">Price ${p.price}</p>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center justify-around w-full sm:pl-28 lg:pl-0">
                      <div className="flex flex-col justify-center items-start max-sm:items-center">
                        ...
                        <p className={`font-semibold text-lg leading-8 text-left whitespace-nowrap ${ord?.status === 'Delivered' ? 'text-green-500' : 'text-red-500'}`}>
                          {ord?.status}
                        </p>
                      </div>
                      <div className="flex flex-col justify-center items-start max-sm:items-center">
                        <p className="font-normal text-lg text-gray-500 leading-8 mb-2 text-left whitespace-nowrap">Delivery Expected by</p>
                        <p className="font-semibold text-lg leading-8 text-black text-left whitespace-nowrap">{new Date(ord?.deliveryDate).toLocaleDateString()}</p>
                      </div>
                    </div>
                  </div>
                ))}
                ...
                <div className="px-3 md:px-11 flex items-center justify-between max-sm:flex-col-reverse">
                  ...
                  <p className="font-medium text-xl leading-8 text-black max-sm:py-4">
                    <span className="text-gray-500">Total Price: </span> &nbsp;${ord?.totalPrice}
                  </p>
                </div>
              </React.Fragment>
            ))}
